...
cities.push(new City ("39","Atlanta","Georgia","456,002","420,003","+8.57%","133.2 sq mi","344.9 km2","3,154 per sq mi","1,218 km−2","33.7629,-84.4227"));
    ...
    // Sort them by the 2010 census (lastCensus)
    // Separate out the "latLon" field into two fields, lat and lon
    // What was the average population change in %?
    // If you find both the "land area" rank and the "change" rank and take the average of both, what city has the highest rank? 




function sortByCensus(){



	for(var i = 0; i < cities.length - 1; i++){
		for(var j = 0; j < cities.length - i - 1; j++){
			if(cities[j].lastCensus > cities[j+1].lastCensus){
				var temp = cities[j];
				cities[j] = cities [i];
				cities[i] = temp;
			}
	...
	cities.forEach(function(City){
		console.log(`${City.city} ${City.lastCensus}`);
	})
